...
def test_duplicates(df, rp_logger):
...
@pytest.mark.validate_csv
def test_validate_schema(df, validate_schema, rp_logger):
...
def test_age_column_valid(df, rp_logger):
    age_column = df['age']
    for i in age_column:
        assert i > 0 and i < 100

@pytest.mark.validate_csv
def test_email_column_valid(df, rp_logger):
...
def test_active_players(df, id_value, expected_active, rp_logger):
...
def test_active_player(df, rp_logger):
